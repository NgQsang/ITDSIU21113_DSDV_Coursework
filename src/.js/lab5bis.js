    ...
    function (data) {
      ...
      svg
        ...
        const brush = d3.brush()
          ...
          .on("start brush end", brushed);// "brushed" is the function that handles the brushing event
        ...
        // Define brush event function
        function brushed(event) {
          const selection = event.selection;
          svg.selectAll('circle')
            .style('fill', function (d) {
              const cx = xScale(d.Long),
                cy = yScale(d.Lat);
              // Highlight selected circles
              return (selection !== null && selection[0][0] <= cx && cx <= selection[1][0]
                && selection[0][1] <= cy && cy <= selection[1][1]) ? 'yellow' : 'red';
            });
        }
